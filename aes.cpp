#include <iostream>
#include <cstring>

// Constants
const int Nb = 4; // block size
int Nk = 4; // key size
int Nr = 10; // number of rounds

unsigned char sbox[256] = {
    
};

unsigned char inv_sbox[256] = {
    0x52, 0x9, 0x6a, 0xd5, 0x30, 0x36, 0xa5, 0x38, 0xbf, 0x40, 0xa3, 0x9e, 0x81, 0xf3, 0xd7, 0xfb, 
    0x7c, 0xe3, 0x39, 0x82, 0x9b, 0x2f, 0xff, 0x87, 0x34, 0x8e, 0x43, 0x44, 0xc4, 0xde, 0xe9, 0xcb, 
    0x54, 0x7b, 0x94, 0x32, 0xa6, 0xc2, 0x23, 0x3d, 0xee, 0x4c, 0x95, 0xb, 0x42, 0xfa, 0xc3, 0x4e, 
    0x8, 0x2e, 0xa1, 0x66, 0x28, 0xd9, 0x24, 0xb2, 0x76, 0x5b, 0xa2, 0x49, 0x6d, 0x8b, 0xd1, 0x25, 
    0x72, 0xf8, 0xf6, 0x64, 0x86, 0x68, 0x98, 0x16, 0xd4, 0xa4, 0x5c, 0xcc, 0x5d, 0x65, 0xb6, 0x92, 
    0x6c, 0x70, 0x48, 0x50, 0xfd, 0xed, 0xb9, 0xda, 0x5e, 0x15, 0x46, 0x57, 0xa7, 0x8d, 0x9d, 0x84, 
    0x90, 0xd8, 0xab, 0x0, 0x8c, 0xbc, 0xd3, 0xa, 0xf7, 0xe4, 0x58, 0x5, 0xb8, 0xb3, 0x45, 0x6, 
    0xd0, 0x2c, 0x1e, 0x8f, 0xca, 0x3f, 0xf, 0x2, 0xc1, 0xaf, 0xbd, 0x3, 0x1, 0x13, 0x8a, 0x6b, 
    0x3a, 0x91, 0x11, 0x41, 0x4f, 0x67, 0xdc, 0xea, 0x97, 0xf2, 0xcf, 0xce, 0xf0, 0xb4, 0xe6, 0x73, 
    0x96, 0xac, 0x74, 0x22, 0xe7, 0xad, 0x35, 0x85, 0xe2, 0xf9, 0x37, 0xe8, 0x1c, 0x75, 0xdf, 0x6e, 
    0x47, 0xf1, 0x1a, 0x71, 0x1d, 0x29, 0xc5, 0x89, 0x6f, 0xb7, 0x62, 0xe, 0xaa, 0x18, 0xbe, 0x1b, 
    0xfc, 0x56, 0x3e, 0x4b, 0xc6, 0xd2, 0x79, 0x20, 0x9a, 0xdb, 0xc0, 0xfe, 0x78, 0xcd, 0x5a, 0xf4, 
    0x1f, 0xdd, 0xa8, 0x33, 0x88, 0x7, 0xc7, 0x31, 0xb1, 0x12, 0x10, 0x59, 0x27, 0x80, 0xec, 0x5f, 
    0x60, 0x51, 0x7f, 0xa9, 0x19, 0xb5, 0x4a, 0xd, 0x2d, 0xe5, 0x7a, 0x9f, 0x93, 0xc9, 0x9c, 0xef, 
    0xa0, 0xe0, 0x3b, 0x4d, 0xae, 0x2a, 0xf5, 0xb0, 0xc8, 0xeb, 0xbb, 0x3c, 0x83, 0x53, 0x99, 0x61, 
    0x17, 0x2b, 0x4, 0x7e, 0xba, 0x77, 0xd6, 0x26, 0xe1, 0x69, 0x14, 0x63, 0x55, 0x21, 0xc, 0x7d
};

unsigned char Rcon[11] = { 0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36 };


void KeyExpansion(unsigned char* inputKey, unsigned char* expandedKeys) {
    // The first 16 bytes are the original key
    for (int i = 0; i < 16; i++) {
        expandedKeys[i] = inputKey[i];
    }

    // Variables
    int bytes_added = 16; // We've added 16 bytes so far
    int rcon_itr = 1;    // Rcon Iteration begins at 1
    unsigned char temp[4];    // Temporary storage for core

    while (bytes_added < 176) {
        // Read 4 bytes for the core
        for (int i = 0; i < 4; i++) {
            temp[i] = expandedKeys[i + bytes_added - 4];
        }

        // Rotate left
        unsigned char t = temp[0];
        temp[0] = temp[1];
        temp[1] = temp[2];
        temp[2] = temp[3];
        temp[3] = t;

        // S-box four bytes
        temp[0] = sbox[temp[0]];
        temp[1] = sbox[temp[1]];
        temp[2] = sbox[temp[2]];
        temp[3] = sbox[temp[3]];

        // Rcon
        temp[0] ^= Rcon[rcon_itr++];

        for (unsigned char a = 0; a < 4; a++) {
            expandedKeys[bytes_added] = expandedKeys[bytes_added - 16] ^ temp[a];
            bytes_added++;
        }
    }
}

void AddRoundKey(unsigned char* state, unsigned char* roundKey) {
    for (int i = 0; i < 16; i++) {
        state[i] ^= roundKey[i];
    }
}

void SubBytes(unsigned char* state) {
    for (int i = 0; i < 16; i++) {
        state[i] = sbox[state[i]];
    }
}

void InvSubBytes(unsigned char* state) {
    for (int i = 0; i < 16; i++) {
        state[i] = inv_sbox[state[i]];
    }
}

void ShiftRows(unsigned char* state) {
    unsigned char tmp[16];

    // Copy the state into a temporary array to make shifting easier
    std::memcpy(tmp, state, 16);

    // Row 0 remains unchanged
    // Row 1
    state[1] = tmp[5];
    state[5] = tmp[9];
    state[9] = tmp[13];
    state[13] = tmp[1];

    // Row 2
    state[2] = tmp[10];
    state[6] = tmp[14];
    state[10] = tmp[2];
    state[14] = tmp[6];

    // Row 3
    state[3] = tmp[15];
    state[7] = tmp[3];
    state[11] = tmp[7];
    state[15] = tmp[11];
}

void InvShiftRows(unsigned char* state) {
    unsigned char tmp[16];

    // Copy the state into a temporary array to make shifting easier
    std::memcpy(tmp, state, 16);

    // Row 0 remains unchanged
    // Row 1 (Shift right by 1)
    state[1] = tmp[13];
    state[5] = tmp[1];
    state[9] = tmp[5];
    state[13] = tmp[9];

    // Row 2 (Shift right by 2)
    state[2] = tmp[10];
    state[6] = tmp[14];
    state[10] = tmp[2];
    state[14] = tmp[6];

    // Row 3 (Shift right by 3)
    state[3] = tmp[7];
    state[7] = tmp[11];
    state[11] = tmp[15];
    state[15] = tmp[3];
}

//Galois multiplication
unsigned char gmul(unsigned char a, unsigned char b) {
    unsigned char p = 0;
    for (int i = 0; i < 8; i++) {
        if (b & 0x01) {
            p ^= a;
        }

        bool high_bit_set = (a & 0x80) != 0;
        a <<= 1;
        if (high_bit_set) {
            a ^= 0x1b; // Modulo operation with irreducible polynomial (x^8 + x^4 + x^3 + x + 1)
        }
        b >>= 1;
    }
    return p;
}

void MixColumns(unsigned char* state) {
    unsigned char temp[16];
    for (int i = 0; i < 4; i++) {
        temp[4*i + 0] = gmul(0x02, state[4*i + 0]) ^ gmul(0x03, state[4*i + 1]) ^ state[4*i + 2] ^ state[4*i + 3];
        temp[4*i + 1] = state[4*i + 0] ^ gmul(0x02, state[4*i + 1]) ^ gmul(0x03, state[4*i + 2]) ^ state[4*i + 3];
        temp[4*i + 2] = state[4*i + 0] ^ state[4*i + 1] ^ gmul(0x02, state[4*i + 2]) ^ gmul(0x03, state[4*i + 3]);
        temp[4*i + 3] = gmul(0x03, state[4*i + 0]) ^ state[4*i + 1] ^ state[4*i + 2] ^ gmul(0x02, state[4*i + 3]);
    }
    memcpy(state, temp, 16);
}

void InvMixColumns(unsigned char* state) {
    unsigned char temp[16];
    for (int i = 0; i < 4; i++) {
        temp[4*i + 0] = gmul(0x0e, state[4*i + 0]) ^ gmul(0x0b, state[4*i + 1]) ^ gmul(0x0d, state[4*i + 2]) ^ gmul(0x09, state[4*i + 3]);
        temp[4*i + 1] = gmul(0x09, state[4*i + 0]) ^ gmul(0x0e, state[4*i + 1]) ^ gmul(0x0b, state[4*i + 2]) ^ gmul(0x0d, state[4*i + 3]);
        temp[4*i + 2] = gmul(0x0d, state[4*i + 0]) ^ gmul(0x09, state[4*i + 1]) ^ gmul(0x0e, state[4*i + 2]) ^ gmul(0x0b, state[4*i + 3]);
        temp[4*i + 3] = gmul(0x0b, state[4*i + 0]) ^ gmul(0x0d, state[4*i + 1]) ^ gmul(0x09, state[4*i + 2]) ^ gmul(0x0e, state[4*i + 3]);
    }
    memcpy(state, temp, 16);
}

void Encrypt(const unsigned char* plaintext, unsigned char* ciphertext, unsigned char* key) {
    unsigned char state[16];
    memcpy(state, plaintext, 16);
    unsigned char expandedKeys[176];
    KeyExpansion(key, expandedKeys);

    AddRoundKey(state, expandedKeys); // Initial round key addition

    for (int i = 0; i < Nr - 1; i++) {
        SubBytes(state);
        ShiftRows(state);
        MixColumns(state);
        AddRoundKey(state, expandedKeys + (16 * (i + 1)));
    }

    // Final round (no MixColumns)
    SubBytes(state);
    ShiftRows(state);
    AddRoundKey(state, expandedKeys + 160);

    memcpy(ciphertext, state, 16);
}

void Decrypt(const unsigned char* ciphertext, unsigned char* decryptedText, unsigned char* key) {
    unsigned char state[16];
    memcpy(state, ciphertext, 16);
    unsigned char expandedKeys[176];
    KeyExpansion(key, expandedKeys);

    AddRoundKey(state, expandedKeys + 160); // Initial round key for decryption

    for (int i = Nr - 1; i > 0; i--) {
        InvShiftRows(state);
        InvSubBytes(state);
        AddRoundKey(state, expandedKeys + (16 * i));
        InvMixColumns(state);
    }

    // Final round (no InvMixColumns)
    InvShiftRows(state);
    InvSubBytes(state);
    AddRoundKey(state, expandedKeys);

    memcpy(decryptedText, state, 16);
}

// int main() {
//     unsigned char plaintext[16] = {'D', 'x', 'a', 'm', 'p', 'l', 'e', ' ', 'p', 'l', 'a', 'i', 'n', 't', 'e', 'x'};
//     unsigned char ciphertext[16];
//     unsigned char decryptedtext[16];
//     unsigned char key[16] = {0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c};

//     // Print original plaintext
//     std::cout << "Original Plaintext:\n";
//     for (int i = 0; i < 16; i++) {
//         printf("%02x ", plaintext[i]);
//     }
//     std::cout << "\n";

//     // Encryption
//     Encrypt(plaintext, ciphertext, key);
//     std::cout << "Ciphertext:\n";
//     for (int i = 0; i < 16; i++) {
//         printf("%02x ", ciphertext[i]);
//     }
//     std::cout << "\n";

//     // Decryption
//     Decrypt(ciphertext, decryptedtext, key);
//     std::cout << "Decrypted Text:\n";
//     for (int i = 0; i < 16; i++) {
//         printf("%02x ", decryptedtext[i]);
//     }
//     std::cout << "\n";

//     return 0;
// }